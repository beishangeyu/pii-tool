...
class TestPiiAnalyzer:
    def test_analysis(self):
        analysis = PiiAnalyzer(
            os.path.abspath("tests/files/titanic_data.csv")
        ).analysis()
        assert ("Nelson" in analysis["people"]) == True
        assert ("William" in analysis["people"]) == False

    def test_analysis_csv_with_unicode(self):
        analysis = PiiAnalyzer(
            os.path.abspath("tests/files/titanic_data_with_unicode.csv")
        ).analysis()
        assert ("Nelson" in analysis["people"]) == True
